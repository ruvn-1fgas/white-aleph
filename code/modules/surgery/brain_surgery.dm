...
/datum/surgery_step/fix_brain/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	display_results(
		user,
		target,
		span_notice("Начинаю исправлять мозг [skloname(target.name, RODITELNI, target.gender)]...") ,
...
/datum/surgery_step/fix_brain/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	display_results(
		user,
		target,
		span_notice("Успешно исправил[user.ru_a()] мозг [skloname(target.name, RODITELNI, target.gender)].") ,
...
/datum/surgery_step/fix_brain/failure(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	if(target.get_organ_slot(ORGAN_SLOT_BRAIN))
		display_results(
			user,
			target,
			span_warning("[gvorno(TRUE)], но я облажался, нанеся еще больший ущерб!") ,
		...
		user.visible_message(span_warning("[user] внезапно замечает что мозг над которым [user.ru_who()] работал[user.ru_a()] пропал.") , span_warning("Неожиданно обнаруживаю что мозг, над которым я работал, исчез."))
	return FALSE
