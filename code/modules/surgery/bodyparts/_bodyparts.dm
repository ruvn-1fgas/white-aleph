...
/obj/item/bodypart/proc/seep_gauze(seep_amt = 0)
	...
	if(current_gauze.absorption_capacity <= 0)
		owner.visible_message(span_danger(" [current_gauze.name] on [owner]'s [name] falls away in rags."), span_warning(" [current_gauze.name] on your [name] falls away in rags."), vision_distance=COMBAT_MESSAGE_RANGE)
		QDEL_NULL(current_gauze)
