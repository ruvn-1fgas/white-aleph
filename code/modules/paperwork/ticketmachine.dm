...
/obj/machinery/ticket_machine/emag_act(mob/user, obj/item/card/emag/emag_card) //Emag the ticket machine to dispense burning tickets, as well as randomize its number to destroy the HoP's mind.
	...
	if(tickets.len)
		for(var/obj/item/ticket_machine_ticket/ticket in tickets)
			ticket.audible_message(span_notice(" [ticket] disperses!"), hearing_distance = SAMETILE_MESSAGE_RANGE)
			qdel(ticket)
		tickets.Cut()
	update_appearance()
	return TRUE
...
/obj/machinery/ticket_machine/proc/increment()
	if(!(obj_flags & EMAGGED) && current_ticket)
		current_ticket.audible_message(span_notice(" [current_ticket] disperses!"), hearing_distance = SAMETILE_MESSAGE_RANGE)
		tickets.Cut(1,2)
	...
	if(LAZYLEN(tickets))
		...
		if(!(obj_flags & EMAGGED))
			current_ticket.audible_message(span_notice(" [current_ticket] vibrates!"), hearing_distance = SAMETILE_MESSAGE_RANGE)
