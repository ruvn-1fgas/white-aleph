...
/obj/item/wormhole_jaunter/attack_self(mob/user)
	user.visible_message(span_notice("[user.name] активирует [src.name]!"))
...
/obj/item/wormhole_jaunter/proc/turf_check(mob/user)
	...
	if(!device_turf || is_centcom_level(device_turf.z) || is_reserved_level(device_turf.z))
		if(user)
			to_chat(user, span_notice("Какие-то трудности у меня с [src.name] to work."))
		return FALSE
	return TRUE
...
/obj/item/wormhole_jaunter/proc/activate(mob/user, adjacent, teleport)
	...
	if(!can_jaunter_teleport())
		if(user)
			to_chat(user, span_notice(" [src] не может найти маяков."))
		else
			visible_message(span_notice(" [src] не может найти маяков!"))
...
/obj/effect/portal/jaunt_tunnel
	name = "червоточина"
	desc = "Стабилизированная дыра в пространстве, созданная генератором червоточин. Турбулентность - это не единственное, с чем вы столкнётесь, но по крайней мере, вы всегда окажетесь где-то рядом с маяком."
