...
/obj/machinery/power/supermatter_crystal/attackby(obj/item/item, mob/user, params)
	if(istype(item, /obj/item/scalpel/supermatter))
		...
		to_chat(user, span_notice("You carefully begin to scrape  [src] with  [scalpel]..."))
		if(!scalpel.use_tool(src, user, 60, volume=100))
			return
		if (scalpel.usesLeft)
			to_chat(user, span_danger("You extract a sliver from  [src].  [src] begins to react violently!"))
			...
			scalpel.usesLeft--
			if (!scalpel.usesLeft)
				to_chat(user, span_notice("A tiny piece of  [scalpel] falls off, rendering it useless!"))
		else
			to_chat(user, span_warning("You fail to extract a sliver from  [src]!  [scalpel] isn't sharp enough anymore."))
		return
	...
	if(istype(item, /obj/item/destabilizing_crystal))
		...
		if(!is_main_engine)
			to_chat(user, span_warning("You can't use  [destabilizing_crystal] on \a [name]."))
			return

		if(get_integrity_percent() < SUPERMATTER_CASCADE_PERCENT)
			to_chat(user, span_warning("You can only apply  [destabilizing_crystal] to \a [name] that is at least [SUPERMATTER_CASCADE_PERCENT]% intact."))
			return

		to_chat(user, span_warning("You begin to attach  [destabilizing_crystal] to  [src]..."))
		if(do_after(user, 3 SECONDS, src))
			...
			to_chat(user, span_danger(" [destabilizing_crystal] snaps onto  [src]."))
