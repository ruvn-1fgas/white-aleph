...
/obj/machinery/power/rtg/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += span_notice("Дисплей: Power generation now at <b>[power_gen*0.001]</b>kW.")
...
/obj/machinery/power/rtg/abductor/proc/overload()
	if(going_kaboom)
		return
	going_kaboom = TRUE
	visible_message(span_danger(" [src] lets out a shower of sparks as it starts to lose stability!"),\
