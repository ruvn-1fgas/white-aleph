...
/datum/computer_file/program/proc/is_supported_by_hardware(hardware_flag = NONE, loud = FALSE, mob/user)
	if(!(hardware_flag & usage_flags))
		if(loud && computer && user)
			to_chat(user, span_danger(" [computer] flashes a \"Hardware Error - Incompatible software\" warning."))
		return FALSE
	return TRUE
...
/datum/computer_file/program/proc/can_run(mob/user, loud = FALSE, access_to_check, transfer = FALSE, list/access)
	...
	if(!length(access))
		...
		if(!accesscard)
			if(loud)
				to_chat(user, span_danger(" [computer] flashes an \"RFID Error - Unable to scan ID\" warning."))
			return FALSE
	...
	if(loud)
		to_chat(user, span_danger(" [computer] flashes an \"Access Denied\" warning."))
	return FALSE
