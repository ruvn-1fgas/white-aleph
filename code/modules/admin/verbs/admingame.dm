...
/datum/admins/proc/show_player_panel(mob/M in GLOB.mob_list)
	...
	body += "<body><b>[M]</b>"
	if(M.client)
		body += " с кеем by <b>[M.client]</b> "
		body += "\[<A href='?_src_=holder;[HrefToken()];editrights=[(GLOB.admin_datums[M.client.ckey] || GLOB.deadmins[M.client.ckey]) ? "rank" : "add"];key=[M.key]'>[M.client.holder ? M.client.holder.rank_names() : "Игрок"]</A>\]"
	...
	if(M.ckey)
		body += "<br>\[<A href='?_src_=holder;[HrefToken()];ppbyckey=[M.ckey];ppbyckeyorigmob=[REF(M)]'>Найти обновленную панель.</A>\]"

	if(M.client)
		body += "<br>\[<b>Наша база:</b> [M.client.player_join_date]\]\[<b>Бьёнд:</b> [M.client.account_join_date]\]"
		body += "<br><br><b>Баны у пиндосов: </b> "
		if(CONFIG_GET(string/centcom_ban_db))
			body += "<a href='?_src_=holder;[HrefToken()];centcomlookup=[M.client.ckey]'>Поиск</a>"
		else
			body += "<i>Выкл?</i>"
		body += "<br><br><b>Мультиакки:</b> "
	...
	body += "<b>Кара: <A href='?_src_=holder;[HrefToken()];boot2=[REF(M)]'>Кик</A> | "
	if(M.client)
	...
	if(M.client)
		body += "| <A href='?_src_=holder;[HrefToken()];sendtoprison=[REF(M)]'>Перма ЦК</A> | "
		...
		body += "(<A href='?_src_=holder;[HrefToken()];mute=[M.ckey];mute_type=[MUTE_ALL]'><font color='[(muted & MUTE_ALL)?"red":"blue"]'>ВСЁ</font></a>)"
	...
	if(M.client)
		if(!isnewplayer(M))
			...
			if(ishuman(M) && !ismonkey(M))
				...
				body += "<A href='?_src_=holder;[HrefToken()];simplemake=human;mob=[REF(M)]'>Человек</A> | "

			if(ismonkey(M))
				body += "<b>Мартышка</b> | "
			else
				body += "<A href='?_src_=holder;[HrefToken()];simplemake=monkey;mob=[REF(M)]'>Мартышка</A> | "
			...
			if(iscyborg(M))
				...
				body += "<A href='?_src_=holder;[HrefToken()];simplemake=robot;mob=[REF(M)]'>Борг</A> | "
