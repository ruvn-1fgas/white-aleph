...
/obj/machinery/food_cart/examine(mob/user)
	...
	if(!(machine_stat & BROKEN))
		if(cart_griddle.machine_stat & BROKEN)
			. += span_warning("<b>Гридль</b> полностью сломан!")
		else
			. += span_notice("<b>Гридль</b> в порядке.")
		. += span_notice("<b>fridge</b> в порядке.") //weirdly enough, these fridges don't break
		. += span_notice("<b>table</b> в порядке.")
...
/obj/machinery/food_cart/proc/unpack(mob/user)
	...
	if(!check_setup_place())
		...
		return
	visible_message(span_notice("[src] раскладывается."))
	set_anchored(TRUE)
	var/iteration = 1
...
/obj/machinery/food_cart/attack_hand(mob/living/user, list/modifiers)
	...
	if(!do_after(user, 5 SECONDS, src))
		to_chat(user, span_warning("[unpacked ? "Распаковка" :"Сборка"] тележки была прервана [src]!"))
		return
	if(unpacked)
		pack_up()
	else
