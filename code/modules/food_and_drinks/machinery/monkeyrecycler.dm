...
/obj/machinery/monkey_recycler/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += span_notice("Дисплей: Производит <b>[cube_production]</b> кубов за каждую обезьяну.")
...
/obj/machinery/monkey_recycler/interact(mob/user)
	if(stored_matter >= 1)
		to_chat(user, span_notice("TМашина громко шипит, перерабатывая обезьянье мясо. Через мгновение она выдает новый обезьяний кубик."))
...
/obj/machinery/monkey_recycler/multitool_act(mob/living/user, obj/item/multitool/I)
	...
	if(istype(I))
		I.set_buffer(src)
		balloon_alert(user, "Записываю [src] в буфер мультитула.")
		return TRUE
