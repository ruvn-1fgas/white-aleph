...
/datum/component/stove/proc/toggle_mode()
	...
	real_parent.balloon_alert_to_viewers("Конфорка [on ? "включена" : "выключена"]")
...
/datum/component/stove/proc/on_attackby(obj/machinery/source, obj/item/attacking_item, mob/user, params)
	...
	if(!attacking_item.is_open_container())
		return
	if(!isnull(container))
		to_chat(user, span_warning("Я не посмею попробовать приготовить две вещи на одной плите одновременно. \
			Что если оно смешается?"))
		return COMPONENT_NO_AFTERATTACK

	if(user.transferItemToLoc(attacking_item, parent))
		add_container(attacking_item, user)
		to_chat(user, span_notice("Поставил [attacking_item] в [parent]."))
	return COMPONENT_NO_AFTERATTACK
...
/datum/component/stove/proc/on_requesting_context(obj/machinery/source, list/context, obj/item/held_item)
	...
	if(isnull(held_item))
		context[SCREENTIP_CONTEXT_RMB] = "[on ? "Выключил":"Включил"] конфорку"
...
/datum/component/stove/proc/on_examine(obj/machinery/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("Могу [on ? "выключить" : "включить"] конфорку <i>правым кликом</i>.")
