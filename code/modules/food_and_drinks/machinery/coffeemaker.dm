...
/obj/machinery/coffeemaker/examine(mob/user)
	...
	if(!in_range(user, src) && !issilicon(user) && !isobserver(user))
		. += span_warning("Слишком далеко для взаимодействия с содержанием [src] и экрану!")
		return

	if(brewing)
		. += span_warning("[src] заваривает.")
		return

	if(panel_open)
		. += span_notice("Техническая панель [src] открыта!")
		return
	...
	if(!(machine_stat & (NOPOWER|BROKEN)))
		. += "[span_notice("Экран статуса показывает:")]\n"+\
		span_notice("- Скорость варки кофе <b>[speed*100]%</b>.")
		if(coffeepot)
			for(var/datum/reagent/consumable/cawfee as anything in coffeepot.reagents.reagent_list)
				. += span_notice("- [cawfee.volume] единиц кофе в чашке.")
		if(cartridge)
			if(cartridge.charges < 1)
				. += span_notice("- картридж пустой.")
			else
				. += span_notice("- картридж имеет [cartridge.charges] зарядов.")

	if (coffee_cups >= 1)
		. += span_notice("Кофейных кашек осталось [coffee_cups].")
	else
		. += span_notice("Кашки кончились.")

	if (sugar_packs >= 1)
		. += span_notice("Пакетиков Сахара осталось [sugar_packs].")
	else
		. += span_notice("Сахар кончился.")

	if (sweetener_packs >= 1)
		. += span_notice("Пакетиков cахарозаменителя осталось [sweetener_packs].")
	else
		. += span_notice("Сахарозаменитель кончился.")

	if (creamer_packs > 1)
		. += span_notice("Упаковок сливок осталось [creamer_packs].")
	else
		. += span_notice("Сливки кончиились.")
...
/obj/machinery/coffeemaker/proc/replace_pot(mob/living/user, obj/item/reagent_containers/cup/coffeepot/new_coffeepot)
	...
	balloon_alert(user, "Чашки установлены")
	update_appearance(UPDATE_OVERLAYS)
	return TRUE
...
/obj/machinery/coffeemaker/attackby(obj/item/attack_item, mob/living/user, params)
	...
	if (istype(attack_item, /obj/item/reagent_containers/cup/glass/coffee_cup) && !(attack_item.item_flags & ABSTRACT) && attack_item.is_open_container())
		...
		if(coffee_cups >= max_coffee_cups)
			balloon_alert(user, "Подстаканник полон!")
			return
	...
	if (istype(attack_item, /obj/item/reagent_containers/condiment/pack/sugar))
		...
		if(new_pack.reagents.total_volume < new_pack.reagents.maximum_volume)
			balloon_alert(user, "Упаковка должна быть полная!")
			return
		if(sugar_packs >= max_sugar_packs)
			balloon_alert(user, "Контейнер сахара полон!")
			return
		if(!user.transferItemToLoc(attack_item, src))
			return
		sugar_packs++
	...
	if (istype(attack_item, /obj/item/reagent_containers/condiment/creamer))
		...
		if(creamer_packs >= max_creamer_packs)
			balloon_alert(user, "Контейнер сливок полон!")
			return
		if(!user.transferItemToLoc(attack_item, src))
			return
		creamer_packs++
	...
	if (istype(attack_item, /obj/item/reagent_containers/condiment/pack/astrotame))
		...
		if(new_pack.reagents.total_volume < new_pack.reagents.maximum_volume)
			...
			balloon_alert(user, "Контейнер сахарозаменителя полон!")
			return
...
/obj/machinery/coffeemaker/proc/try_brew()
	if(!cartridge)
		balloon_alert(usr, "картриджа нет!")
		return FALSE
	if(cartridge.charges < 1)
		balloon_alert(usr, "картридж пустой!")
		return FALSE
	if(!coffeepot)
		balloon_alert(usr, "Стаканов нет!")
		return FALSE
	if(machine_stat & (NOPOWER|BROKEN))
		balloon_alert(usr, "Машина обесточена!")
		return FALSE
	if(coffeepot.reagents.total_volume >= coffeepot.reagents.maximum_volume)
		balloon_alert(usr, "Чашка уже полная!")
		return FALSE
	return TRUE
...
/obj/machinery/coffeemaker/ui_interact(mob/user) // The microwave Menu //I am reasonably certain that this is not a microwave //I am positively certain that this is not a microwave
	...
	if(coffeepot)
		options["Вытащить чашки"] = radial_eject_pot

	if(cartridge)
		options["Вытащить картридж"] = radial_eject_cartridge
...
/obj/machinery/coffeemaker/proc/take_sugar(mob/user)
	if(!sugar_packs)
		balloon_alert(user, "Сахар кончился!")
		return
...
/obj/machinery/coffeemaker/proc/take_sweetener(mob/user)
	if(!sweetener_packs)
		balloon_alert(user, "Сахарозаменитель кончился!")
		return
...
/obj/machinery/coffeemaker/proc/take_creamer(mob/user)
	if(!creamer_packs)
		balloon_alert(user, "Сливки кончились!")
		return
...
/obj/item/coffee_cartridge
	name = "картридж кофеварки - Caffè Generico"
	desc = "Кофейный картридж производства Piccionaia Coffee для использования с системой Modello 3."
...
/obj/item/coffee_cartridge/examine(mob/user)
	. = ..()
	if(charges)
		. += span_warning("Картридж имеет [charges] порций кофе.")
	else
		. += span_warning("Картридж пустой.")

/obj/item/coffee_cartridge/fancy
	name = "картридж кофеварки - Caffè Fantasioso"
	desc = "Дорогой элитный кофейный картридж производства Piccionaia Coffee для использования с системой Modello 3."
...
/obj/item/coffee_cartridge/fancy/Initialize(mapload)
	...
	switch(coffee_type)
		if("blend")
			name = "картридж для кофе - Miscela di Piccione"
			icon_state = "cartridge_blend"
		if("blue_mountain")
			name = "картридж для кофе - Montagna Blu"
			icon_state = "cartridge_blue_mtn"
		if("kilimanjaro")
			name = "картридж для кофе - Kilimangiaro"
			icon_state = "cartridge_kilimanjaro"
		if("mocha")
			name = "картридж для кофе - Moka Arabica"
			icon_state = "cartridge_mocha"

/obj/item/coffee_cartridge/decaf
	name = "картридж кофеварки - Caffè Decaffeinato"
	desc = "Кофейный картридж без кофеина производства Piccionaia Coffee, для использования с системой Modello 3."
	icon_state = "cartridge_decaf"

// no you can't just squeeze the juice bag into a glass!
/obj/item/coffee_cartridge/bootleg
	name = "картридж кофеварки - Botany Blend"
	desc = "Кофейный картридж, сделанный на скорую руку. Должен работать с системой Modello 3, хотя он может аннулировать гарантию."
...
/obj/item/blank_coffee_cartridge
	name = "пустой картридж для кофе"
	desc = "Чистый кофейный картридж, готовый к заполнению кофейной пастой."
	icon = 'icons/obj/food/cartridges.dmi'
...
/obj/item/storage/fancy/coffee_cart_rack
	name = "стойка для картриджей с кофе"
	desc = "Небольшая стойка для хранения картриджей для кофеварки."
...
/obj/machinery/coffeemaker/impressa
	...
	desc = "Кофеварка Impressa Modello 5 производства Piccionaia Home Appliances. Делает кофе из свежих сушеных бобов."
...
/obj/machinery/coffeemaker/impressa/examine(mob/user)
	. = ..()
	if(coffee)
		. += span_notice("Внутренний дробитель имеет [coffee.len] кофейных зерен")
...
/obj/machinery/coffeemaker/impressa/try_brew()
	if(coffee_amount <= 0)
		...
		return FALSE
	if(!coffeepot)
		balloon_alert_to_viewers("Нет кофейных чашек!")
		return FALSE
	if(machine_stat & (NOPOWER|BROKEN) )
		balloon_alert_to_viewers("Машина обесточена!")
		return FALSE
	if(coffeepot.reagents.total_volume >= coffeepot.reagents.maximum_volume)
		balloon_alert_to_viewers("Чашка уже полная!")
		return FALSE
	return TRUE
...
/obj/machinery/coffeemaker/impressa/attackby(obj/item/attack_item, mob/living/user, params)
	...
	if (istype(attack_item, /obj/item/reagent_containers/cup/glass/coffee) && !(attack_item.item_flags & ABSTRACT) && attack_item.is_open_container())
		...
		if(new_cup.reagents.total_volume > 0 )
			balloon_alert(user, "Чашка должна быть пустая!")
			return
		if(coffee_cups >= max_coffee_cups)
			balloon_alert(user, "Подстаканник полный!")
			return
		if(!user.transferItemToLoc(attack_item, src))
			return
		coffee_cups++
	...
	if (istype(attack_item, /obj/item/reagent_containers/condiment/pack/sugar))
		var/obj/item/reagent_containers/condiment/pack/sugar/new_pack = attack_item
		if(new_pack.reagents.total_volume < new_pack.reagents.maximum_volume)
			balloon_alert(user, "Упаковка должна быть полной!")
			return
		if(sugar_packs >= max_sugar_packs)
			balloon_alert(user, "Контейнер сахара полон")
			return
	...
	if (istype(attack_item, /obj/item/reagent_containers/condiment/pack/astrotame))
		...
		if(sweetener_packs >= max_sweetener_packs)
			balloon_alert(user, "Контейнер сахаразаменителя полон!")
			return
		if(!user.transferItemToLoc(attack_item, src))
			return
		sweetener_packs++
	...
	if (istype(attack_item, /obj/item/food/grown/coffee) && !(attack_item.item_flags & ABSTRACT))
		if(coffee_amount >= BEAN_CAPACITY)
			balloon_alert(user, "Контейнер кофе полон!")
			return
		if(!HAS_TRAIT(attack_item, TRAIT_DRIED))
			balloon_alert(user, "Кофейные зерна должны быть высушенные!")
			return
	...
	if (istype(attack_item, /obj/item/storage/box/coffeepack))
		if(coffee_amount >= BEAN_CAPACITY)
			balloon_alert(user, "Контейнер кофе полный!")
			return
		...
		for(var/obj/item/food/grown/coffee/new_coffee in new_coffee_pack.contents)
			if(HAS_TRAIT(new_coffee, TRAIT_DRIED)) //the coffee beans inside must be dry
				...
				balloon_alert(user, "Не высущенные зерна кофе в упаковке!")
				return
...
/obj/machinery/coffeemaker/impressa/take_cup(mob/user)
	if(!coffee_cups) //shouldn't happen, but we all know how stuff manages to break
		balloon_alert(user, "Чашки кончились!")
		return
