...
/obj/machinery/nuclearbomb/proc/arm_nuke(mob/armer)
	...
	message_admins(" [src] was armed at [ADMIN_VERBOSEJMP(our_turf)] by [armer ? ADMIN_LOOKUPFLW(armer) : "an unknown user"].")
	armer.log_message("armed  [src].", LOG_GAME)
...
/obj/machinery/nuclearbomb/proc/disarm_nuke(mob/disarmer)
	var/turf/our_turf = get_turf(src)
	message_admins(" [src] at [ADMIN_VERBOSEJMP(our_turf)] was disarmed by [disarmer ? ADMIN_LOOKUPFLW(disarmer) : "an unknown user"].")
	if(disarmer)
