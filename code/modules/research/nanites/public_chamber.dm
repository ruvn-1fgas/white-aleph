...
/obj/machinery/public_nanite_chamber/close_machine(mob/living/carbon/user, mob/living/attacker, density_to_set = TRUE)
	if(!state_open)
		return FALSE
...
/obj/machinery/public_nanite_chamber/MouseDrop_T(mob/target, mob/user)
	if(!user.can_perform_action(src, NEED_DEXTERITY | NEED_HANDS) || !Adjacent(target) || !user.Adjacent(target) || !iscarbon(target))
		return
