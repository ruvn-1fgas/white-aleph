...
/mob/living/silicon/proc/deadchat_lawchange()
	...
	deadchat_broadcast(" <b>теперь следует новым законам.</b> <a href='?src=[REF(src)]&dead=1&printlawtext=[url_encode(lawtext)]'>Показать</a>", span_name("[src]"), follow_target=src, message_type=DEADCHAT_LAWCHANGE)

/mob/living/silicon/proc/post_lawchange(announce = TRUE)
	throw_alert(ALERT_NEW_LAW, /atom/movable/screen/alert/newlaw)
	if(announce && last_lawchange_announce != world.time)
		to_chat(src, span_boldannounce("Мои законы были изменены."))
