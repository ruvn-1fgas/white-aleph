...
/obj/effect/mob_spawn/ghost_role/drone/Initialize(mapload)
	...
	if(area)
		notify_ghosts("A drone shell has been created in  [area.name].", source = src, action = NOTIFY_PLAY, flashwindow = FALSE, ignore_key = POLL_IGNORE_DRONE, notify_suiciders = FALSE)
