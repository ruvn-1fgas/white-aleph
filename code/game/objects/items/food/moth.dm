...
/obj/item/food/bread/corn/make_processable()
...
/obj/item/food/cake/mothmallow/make_processable()
...
/obj/item/storage/box/gum/wake_up/examine_more(mob/user)
	. = ..()
	. += (span_notice("<i>Читаю описание жвачки на упаковке...</i>"))
	. += "\t[span_info("Для снятия усталости и сонливости во время работы.")]"
	. += "\t[span_info("Не разжевывайте более одной полоски каждые 12 часов. Не используйте в качестве полной замены сна.")]"
	. += "\t[span_info("Не давать детям до 16 лет. Не превышайте максимальную дозировку. Не принимать внутрь. Не принимать более 3 дней подряд. Не принимать в сочетании с другими лекарствами. Может вызывать побочные реакции у пациентов с уже существующими заболеваниями сердца.")]"
	. += "\t[span_info("Побочные эффекты применения Активина могут включать подергивание усиков, гиперактивные крылья, потерю кератинового блеска, потерю покрытия волосков, аритмию, помутнение зрения и эйфорию. При появлении побочных эффектов прием препарата следует прекратить.")]"
	. += "\t[span_info("Повторное нескольких полосок за раз может вызвать зависимость.")]"
	. += "\t[span_info("В случае превышения максимальной дозы немедленно сообщите об этом члену медицинского персонала судна. Не вызывайте рвоту.")]"
	. += "\t[span_info("Состав: каждая полоска содержит 500 мг Активина (декстро-метамфетамин). Другие ингредиенты включают зеленый краситель 450 (Verdant Meadow) и искусственный травяной ароматизатор.")]"
	. += "\t[span_info("Хранение: хранить в сухом прохладном месте. Не использовать после истечения срока годности: 32/4/350.")]"
	return .
