...
/obj/machinery/door/airlock/examine(mob/user)
	...
	if(issilicon(user) && !(machine_stat & BROKEN))
		...
		. += span_notice("Alt-клик по [src], чтобы [ secondsElectrified ? "снять электризацию с н" : "электризовать "]его.")
...
/obj/machinery/door/airlock/add_context(atom/source, list/context, obj/item/held_item, mob/user)
	...
	if(isAI(user) || iscyborg(user))
		if(!(machine_stat & BROKEN))
			...
			context[SCREENTIP_CONTEXT_SHIFT_LMB] = density ? "Открыть" : "Закрыть"
			context[SCREENTIP_CONTEXT_CTRL_LMB] = locked ? "Снять болты" : "Опустить болты"
			context[SCREENTIP_CONTEXT_ALT_LMB] = isElectrified() ? "Снять электризацию" : "Электризовать"
			context[SCREENTIP_CONTEXT_CTRL_SHIFT_LMB] = emergency ? "Отключить экстренный доступ" : "Включить экстренный доступ"
		...
		if (TOOL_CROWBAR)
			if (panel_open)
				if (security_level == AIRLOCK_SECURITY_PLASTEEL_O_S || security_level == AIRLOCK_SECURITY_PLASTEEL_I_S)
					context[SCREENTIP_CONTEXT_LMB] = "Снять щиток"
					return CONTEXTUAL_SCREENTIP_SET
				else if (should_try_removing_electronics())
					context[SCREENTIP_CONTEXT_LMB] = "Достать плату"
					return CONTEXTUAL_SCREENTIP_SET
			...
			context[SCREENTIP_CONTEXT_LMB] = "Поддеть ломом"
			...
			if (panel_open)
				switch (security_level)
					if (AIRLOCK_SECURITY_IRON, AIRLOCK_SECURITY_PLASTEEL_I, AIRLOCK_SECURITY_PLASTEEL_O)
						context[SCREENTIP_CONTEXT_LMB] = "Разрезать внутренний слой защиты"
						return CONTEXTUAL_SCREENTIP_SET
...
/obj/machinery/door/airlock/proc/hack(mob/user)
	...
	if(!aiHacking)
		...
		if(!check_hacking(user, "Неисправность подтверждена: провод управления шлюзом отключен или оборван."))
			return
		sleep(2 SECONDS)

		if(!check_hacking(user, "Попытка взломать шлюз. Это может занять некоторое время.."))
			return
		sleep(20 SECONDS)

		if(!check_hacking(user, "Доступ к загрузке подтвержден. Загрузка программы управления в программное обеспечение шлюза."))
			return
		sleep(17 SECONDS)

		if(!check_hacking(user,"Передача завершена. Принуждаем шлюз выполненить программу."))
			return
		sleep(5 SECONDS)

		if(!check_hacking(user, "Получение контрольной информации из шлюза."))
			return
...
/obj/machinery/door/airlock/wirecutter_act(mob/living/user, obj/item/tool)
	...
	if(note)
		if(user.CanReach(src))
			...
			visible_message(span_notice("[tool] срезает [note] с [src]."))
		tool.play_tool_sound(src)
...
/obj/machinery/door/airlock/crowbar_act(mob/living/user, obj/item/tool)
	...
	switch(security_level)
		if(AIRLOCK_SECURITY_PLASTEEL_O_S)
			layer_flavor = "внешний"
			next_level = AIRLOCK_SECURITY_PLASTEEL_I

		if(AIRLOCK_SECURITY_PLASTEEL_I_S)
			layer_flavor = "внутренний"
			next_level = AIRLOCK_SECURITY_NONE
		else
			return TOOL_ACT_TOOLTYPE_SUCCESS

	user.visible_message(span_notice("Начинаю снимать [layer_flavor] слой защиты."))
	if(!tool.use_tool(src, user, 40, volume=100))
		return TOOL_ACT_TOOLTYPE_SUCCESS
	if(!panel_open || security_level != starting_level)
		// if the plating's already been broken, don't break it again
		return TOOL_ACT_TOOLTYPE_SUCCESS
	user.visible_message(span_notice("[user] снимает [layer_flavor] защитный экран [src]."),
							span_notice("снимаю [layer_flavor] защитный экран [src]."))
...
/obj/machinery/door/airlock/wrench_act(mob/living/user, obj/item/tool)
	...
	if(security_level != AIRLOCK_SECURITY_NONE)
		balloon_alert(user, "шлюз усилен!")
		return

	if(istype(tool, /obj/item/wrench/bolter))
		balloon_alert(user, "поднимаю болты...")
...
/obj/machinery/door/airlock/welder_act(mob/living/user, obj/item/tool)
	...
	switch(security_level)
		if(AIRLOCK_SECURITY_IRON)
			layer_flavor = "разрезать экран панели"
			next_level = AIRLOCK_SECURITY_NONE
			material_to_spawn = /obj/item/stack/sheet/iron
			amount_to_spawn = 2
		if(AIRLOCK_SECURITY_PLASTEEL_O)
			layer_flavor = "резать внутренний слой защиты"
	...
	to_chat(user, span_notice("Начинаю [layer_flavor]..."))
	...
	user.visible_message(
		span_notice("[user] прорезается сквозь защитный экран [src]."),  // passers-by don't get the full picture
		span_notice("Прорезаюсь сквозь защитный экран [src]."),
		span_hear("Слышку сварку.")
	)
...
/obj/machinery/door/airlock/attackby(obj/item/C, mob/user, params)
	...
	if(is_wire_tool(C) && panel_open)
		...
		if(seal)
			...
			return
		user.visible_message(span_notice("[user] начинает блокировать [src]."), span_notice("Начинаю запечатывать [src]."))
...
/obj/machinery/door/airlock/try_to_crowbar(obj/item/I, mob/living/user, forced = FALSE)
	...
	if(!operating)
		if(istype(I, /obj/item/fireaxe) && !HAS_TRAIT(I, TRAIT_WIELDED)) //being fireaxe'd
			to_chat(user, span_warning("Требуется держать [I] в обеих руках для этого!"))
			return
