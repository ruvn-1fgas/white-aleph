...
/atom/movable/screen/lobby/button/join/Click(location, control, params)
	...
	if(!LAZYACCESS(params2list(params), CTRL_CLICK))
		...
		to_chat(new_player, span_warning("Открытие аварийного меню для присоединения к раунду! Если это сообщение не появится, то срочно напишите администратору!"))
...
/atom/movable/screen/lobby/button/collapse/Click(location, control, params)
	...
	name = "[our_hud.menu_hud_status ? "Развернуть" : "Свернуть"] меню"
	set_button_status(FALSE)
