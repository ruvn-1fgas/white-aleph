...
/datum/component/nanites/process(delta_time)
	if(!host_mob.has_status_effect(/datum/status_effect/grouped/stasis)) //Runtime in nanites.dm,115: Cannot read null.researched_nodesproc name: process (/datum/component/nanites/process)
		adjust_nanites(null, (regen_rate + (stored_research.researched_nodes ? HARMONIC_REGEN_BOOST : 0)) * delta_time)
		add_research()
...
/datum/component/nanites/proc/reject_excess_nanites()
	...
	switch(excess)
		...
		if((NANITE_EXCESS_MINOR + 0.1) to NANITE_EXCESS_VOMIT) //Enough nanites getting rejected at once to be visible to the naked eye
			host_mob.visible_message(span_warning("Зернистая серая жижа начинает сочиться из [skloname(host_mob, RODITELNI, host_mob.gender)]."), span_warning("Зернистая серая жижа начинает сочиться из моей кожи."), null, 4);
		if((NANITE_EXCESS_VOMIT + 0.1) to NANITE_EXCESS_BURST) //Nanites getting rejected in massive amounts, but still enough to make a semi-orderly exit through vomit
			if(iscarbon(host_mob))
				...
				host_mob.visible_message(span_warning("[skloname(host_mob, RODITELNI, host_mob.gender)]вырывает зернистой серой массой!"), span_warning("Внезапно тошнит серой металлической массой!"), null);
				C.vomit(0, FALSE, TRUE, FLOOR(excess / 100, 1), FALSE, VOMIT_NANITE, FALSE, TRUE, 0)
			else
				host_mob.visible_message(span_warning("Металлическая серая жижа выходит из кожи [skloname(host_mob, RODITELNI, host_mob.gender)]!"), span_userdanger("Металлическая масса резко вырывается из моей кожи!"), null);
		...
		if((NANITE_EXCESS_BURST + 0.1) to INFINITY) //Way too many nanites, they just leave through the closest exit before they harm/poison the host
			host_mob.visible_message(span_warning("Поток металлической серой навозной жижи резко вырывается из лица и кожи [skloname(host_mob, RODITELNI, host_mob.gender)]!"),
								span_userdanger("Поток металлической серой жижи яростно вылетает из моих глаз, ушей и рта, и вытекает из кожи!"));
			...
			if(iscarbon(host_mob))
				...
				if(ears)
					host_mob.adjustOrganLoss(ORGAN_SLOT_EARS, 30) //nanites coming out of your ears
...
/datum/component/nanites/proc/receive_signal(datum/source, code, source = "неопознанный источник")
	SIGNAL_HANDLER
...
/datum/component/nanites/proc/receive_comm_signal(datum/source, comm_code, comm_message, comm_source = "неопознанный источник")
	SIGNAL_HANDLER
...
/datum/component/nanites/proc/add_research()
	...
	if(host_mob.stat == DEAD)
		research_value *= 0.75
	stored_research.add_point_list(list(TECHWEB_POINT_TYPE_NANITES = research_value))
...
/datum/component/nanites/proc/nanite_scan(datum/source, mob/user, full_scan)
	...
	if(!full_scan)
		if(!stealth)
			to_chat(user, span_notice("<b>НАНИТЫ ОБНАРУЖЕНЫ</b>"))
			to_chat(user, span_notice("Концентрация: [nanite_volume]/[max_nanites]"))
			return TRUE
	else
		to_chat(user, span_info("НАНИТЫ ОБНАРУЖЕНЫ"))
		to_chat(user, span_info("================"))
		to_chat(user, span_info("Концентрация: [nanite_volume]/[max_nanites]"))
		to_chat(user, span_info("Безопасный порог: [safety_threshold]"))
		to_chat(user, span_info("ИД облака: [cloud_id ? cloud_id : "нет"]"))
		to_chat(user, span_info("Синхронизация с облаком: [cloud_active ? "Активна" : "Отключена"]"))
		to_chat(user, span_info("================"))
		to_chat(user, span_info("Список программ:"))
		if(!diagnostics)
			to_chat(user, span_alert("Отладка отключена."))
		else
			for(var/X in programs)
				var/datum/nanite_program/NP = X
				to_chat(user, span_info("<b>[NP.name]</b> | [NP.activated ? "Активна" : "Неактивна"]"))
		return TRUE
...
/datum/component/nanites/proc/nanite_ui_data(datum/source, list/data, scan_level)
	...
	data["regen_rate"] = regen_rate + (stored_research.researched_nodes ? HARMONIC_REGEN_BOOST : 0)
