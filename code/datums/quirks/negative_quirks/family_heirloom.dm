...
/datum/quirk/item_quirk/family_heirloom/add_unique(client/client_source)
	...
	give_item_to_holder(
		...
		flavour_text = "Дорогая для вас реликвия передавалась из поколения в поколение. Хранить в безопасности!"
	)
...
/datum/quirk/item_quirk/family_heirloom/post_add()
	...
	if(!family_heirloom)
		to_chat(quirk_holder, span_boldnotice("Волна экзистенциального страха охватывает меня, приходя с осознанием того что драгоценная семейная реликвия пропала. Возможно, Боги проявят милосердие к моей проклятой душе?"))
		return
