...
/datum/mood/proc/print_mood(mob/user)
	...
	msg += span_notice("Психика: ") //Long term
	switch(sanity)
		if(SANITY_GREAT to INFINITY)
			msg += "[span_boldnicegreen("Мой разум словно храм!")]\n"
		if(SANITY_NEUTRAL to SANITY_GREAT)
			msg += "[span_nicegreen("Чувствую себя прекрасно в последнее время!")]\n"
		if(SANITY_DISTURBED to SANITY_NEUTRAL)
			msg += "[span_nicegreen("Чувствую себя вполне нормально в последнее время..")]\n"
		if(SANITY_UNSTABLE to SANITY_DISTURBED)
			msg += "[span_warning("Немного не в себе...")]\n"
		if(SANITY_CRAZY to SANITY_UNSTABLE)
			msg += "[span_warning("В бешенстве!!")]\n"
		if(SANITY_INSANE to SANITY_CRAZY)
			msg += "[span_boldwarning("АХАХАХАХАХАХАХАХАХ!!")]\n"

	msg += span_notice("Настроение: ") //Short term
	switch(mood_level)
		if(MOOD_LEVEL_SAD4)
			msg += "[span_boldwarning("Хотелось бы мне умереть!")]\n"
		if(MOOD_LEVEL_SAD3)
			msg += "[span_boldwarning("Чувствую себя ужасно...")]\n"
		if(MOOD_LEVEL_SAD2)
			msg += "[span_boldwarning("Мне очень грустно.")]\n"
		if(MOOD_LEVEL_SAD1)
			msg += "[span_warning("Мне грустно.")]\n"
		if(MOOD_LEVEL_NEUTRAL)
			msg += "[span_grey("В порядке.")]\n"
		if(MOOD_LEVEL_HAPPY1)
			msg += "[span_nicegreen("Чувствую себя хорошо.")]\n"
		if(MOOD_LEVEL_HAPPY2)
			msg += "[span_boldnicegreen("Чувствую себя довольно хорошо.")]\n"
		if(MOOD_LEVEL_HAPPY3)
			msg += "[span_boldnicegreen("Чувствую себя потрясающе!")]\n"
		if(MOOD_LEVEL_HAPPY4)
			msg += "[span_boldnicegreen("Люблю жизнь!")]\n"
	...
	if(mood_events.len)
		...
		msg += "[span_grey("Да как-то всё равно на всё в данный момент.")]\n"
