...
/datum/tgs_event_handler/impl/HandleEvent(event_code, ...)
	switch(event_code)
		...
		if(TGS_EVENT_DEPLOYMENT_COMPLETE)
			...
			to_chat(world, span_rose("-- #<b>Сервер обновлен, изменения будут применены в следующем раунде...</b> # --"))
		...
		if(TGS_EVENT_WATCHDOG_REATTACH)
			...
			if(!old_version.Equals(new_version))
				to_chat(world, span_boldannounce("TGS был обновлен до версии [new_version.deprefixed_parameter]"))
			else
		...
		if(TGS_EVENT_WATCHDOG_SHUTDOWN)
			to_chat_immediate(world, span_boldannounce("Сервер отключается!"))
